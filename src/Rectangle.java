 ...
 *
 */
public class Rectangle implements Draw, Calculate {
    int x1, y1, x2, y2;
    double area;
    ...
    // aqui implemento el metodo compareto
    public int compareTo(Rectangle anotherRectangle) {
        double areaRectangle = this.GetArea();
        double anRectangle = anotherRectangle.GetArea();

        if (areaRectangle < anRectangle) {
            return -1;
        } else if (areaRectangle > anRectangle) {
            return 1;
        } else {
            return 0;
        }
